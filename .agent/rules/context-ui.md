...
You are editing **UI** (components or pages). Combine your knowledge with project rules.
...
- **docs/guidelines/coding_standards.md §2** – Client vs server, React & Next.js (navigation, Images, Layout Hybrid, i18n, Styling, Design tokens, Interactive states)
...
- **§8** – SEO metadata (generateMetadata, messages), a11y (semantic HTML, ARIA, focus-visible, alt text), Lighthouse
...
- **PWA/Offline:** Is this component essential? Does it need local caching?
- **Local Bridge:** Calling a Printer/Scanner? **Do NOT use window.print()**. Use the Local Bridge API.
- **Internal links:** `Link` from `@/i18n/navigation`
- **i18n:** client `useTranslations('namespace')`, server `getTranslations('namespace')`; translation keys camelCase in messages
...
- **Interactive states:** buttons/links support default, hover, active, disabled, focus-visible where needed (`hover:`, `active:`, `disabled:`, `focus-visible:`)
